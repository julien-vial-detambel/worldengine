...
def generate_plates_simulation(seed, width, height, axial_tilt, sea_level=0.65,
    ...
    logger.logger.info('...plates.generate_plates_simulation() complete. ' +
...
def world_gen(name, width, height, axial_tilt, seed, temps=[.874, .765, .594, .439, .366, .124],
    ...
    center_land(world)
    elapsed_time = time.time() - start_time
    logger.logger.info('...plates.world_gen: set_elevation, set_plates, ' +
    ...
    elapsed_time = time.time() - start_time
    logger.logger.info('...plates.world_gen: elevation noise added. Elapsed ' +
    ...
    elapsed_time = time.time() - start_time
    logger.logger.info('...plates.world_gen: oceans initialized. Elapsed ' +
                       'time ' + str(elapsed_time) + ' seconds.')
